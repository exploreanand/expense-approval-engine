...
// --- ADMIN: CATEGORY MANAGEMENT (FR-8) ---
...
/**
 * Creates a complex approval policy including thresholds, 
 * sequential approvers, and optional short-circuit rules[cite: 89].
 */
export async function createPolicy(policyData) {
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      // 1. Create the base policy [cite: 134]
      ...
      // 2. Add sequential approvers [cite: 136]
      if (policyData.approverIds?.length > 0) {
        await tx.policyApprover.createMany({
          data: policyData.approverIds.map((id, index) => ({
            policyId: policy.id,
            approverId: id,
            sequence: index + 1
          }))
      ...
      // 3. Add Conditional Short-Circuit Rule [cite: 96, 139]
      if (policyData.rule) {
    ...
    return { success: false, error: "Failed to save policy configuration." };
  }
}

// --- ADMIN: GET POLICIES (FR-5) ---
...
// --- SHARED: GET EXPENSE DETAIL (FR-4) ---
/**
 * Fetches an expense with its full history and category for the timeline view[cite: 174].
 */
...
// --- ADMIN: GET ALL COMPANY AUDIT LOGS (FR-7) ---
export async function getCompanyAuditLogs() {
  ...
  return await prisma.auditLog.findMany({
    ...
    include: { 
      ...
      expense: { select: { description: true, convertedAmount: true } }
    },
    orderBy: { createdAt: 'desc' }
  });
}